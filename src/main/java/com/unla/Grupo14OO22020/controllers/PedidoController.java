...
//		}
	...
	public ModelAndView crearPorlocal() {
		...
		mAV.addObject("empleados",empleadoService.traerEmpleadosDelLocal(local));
...
//	    globalCantidad = pedido.getCantidad();
	...
	public ModelAndView mostrarTodosLosPedDelLocal(Local local) {
		...
		if(!pedidoService.pedidosEntregadosPropios(local).isEmpty()) {mAV.addObject("mensaje1","Ventas con stock propio");}
		if(!pedidoService.pedidosEntregadosAgenos(local).isEmpty()) {mAV.addObject("mensaje2","Ventas con stock ajeno");}
		...
		mAV.addObject("pedidosEntrPropios",pedidoService.pedidosEntregadosPropios(local));
		mAV.addObject("pedidosEntrAgenos",pedidoService.pedidosEntregadosAgenos(local));
...
//		mAV.addObject("locales",localService.getAll());
	...
	public boolean descontarStockDeUnProductoLocalPropio(LocalModel localM, ProductoModel productoM, int cantidadPedida){
		...
		Local local = localRepository.findByIdLocal(localM.getIdLocal());
		Producto producto = productoRepository.findByIdProducto(productoM.getIdProducto());
		seDesconto = descontarStockDeUnProducto(local, producto, cantidadPedida);
		return seDesconto;
	}
	...
	public boolean descontarStockDeUnProductoLocalAjeno(PedidoModel pedido){
		...
		Producto producto = productoRepository.findByIdProducto(pedido.getProducto().getIdProducto());
		seDesconto = descontarStockDeUnProducto(localDelEmplAux, producto, pedido.getCantidad());
	  return seDesconto;
	}
	
	public boolean descontarStockDeUnProducto(Local local, Producto producto, int cantidadPedida){
		boolean seDesconto=false;
		List<Lote> lotesDeunProducto= new ArrayList<Lote>();				
		for(Lote lote : loteRepository.lotesDeUnLocalOrdenadosPorID(local.getIdLocal())){//Lo traje así porque con "local.getLotes()" llega desordenado
			if(lote.getProducto().equals(producto)){//if ... si es del producto
				lotesDeunProducto.add(lote);
				System.out.println(" ID "+lote.getIdLote()+" cantidad "+lote.getCantidadActual() );
			}
		}//fin for 1
				
		for(int i=0;i<lotesDeunProducto.size();i++) {//recorro los lotes del producto
			if(cantidadPedida>0){//si no se descontó la cantidad que me piden
				cantidadPedida = desconcontarPorLoteDeLocal(local, lotesDeunProducto.get(i), cantidadPedida);
			}else if(cantidadPedida==0){ 
			    i = lotesDeunProducto.size();//sale del for
				seDesconto = true;
			}
		}//fin for 4
	  return seDesconto;			
	...
	public int desconcontarPorLoteDeLocal(Local local, Lote lote, int cantidadPedida) {
		if(cantidadPedida==lote.getCantidadActual()) {   // 10  10 
			lote.setCantidadActual(0);//lo pongo en cero
			cantidadPedida = 0;
			actualizarDatosDeLoteYLocal(local,lote);
		}else if(cantidadPedida<lote.getCantidadActual()){                // 30          50
			lote.setCantidadActual(lote.getCantidadActual()-cantidadPedida);
			cantidadPedida =0;
			actualizarDatosDeLoteYLocal(local,lote);
		}else if(cantidadPedida>lote.getCantidadActual()){ //         70    20
			cantidadPedida = cantidadPedida - lote.getCantidadActual();
	...
	public void actualizarDatosDeLoteYLocal(Local local, Lote lote) {
 		lote.setLocal(local);
 		loteService.Update(loteConverter.entityToModel(lote));
 		localService.insertOrUpdate(localConverter.entityToModel(local));
    }//FIN VOID actualizarDatosDeLoteYLocal
	...
	public List<Pedido> pedidosParaEnviarPorLocal(Local local){//no hay stock propio; se tienen que enviar a otro local a la espera de aceptación o rechazo
		...
		for(Pedido pedido: pedidoService.pedidosPorLocal(local)) {
